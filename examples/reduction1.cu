// redukcija se ovako ne bi smjela kodirati
...
{
	double	*data = in_data + blockIdx.x * blockDim.x;

	for (int stride = 1; stride < blockDim.x; stride <<= 1) {
		if ((threadIdx.x == 0) || ((threadIdx.x & -threadIdx.x) > stride))
...
int	main(int argc, char **argv)
{
	...
	init_matrix<double>(hst_array, N, 1, 0);
